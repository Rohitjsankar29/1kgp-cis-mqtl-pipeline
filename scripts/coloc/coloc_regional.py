...
ax[0].axvline(cpos/1e6,color='grey',ls=':',lw=1,alpha=0.6); ax[0].set_ylabel('-log10 P (mQTL)'); ax[0].set_title(f'{gene}  (CpG {cpg}): methylation QTL vs {disease} colocalization',fontweight='bold')
ax[0].legend()
ax[1].scatter(mq.pos/1e6,-np.log10(mq.gwas_p),c=mq.r2,cmap='RdYlBu_r',vmin=0,vmax=1,s=25,edgecolor='grey',linewidth=0.2)
ax[1].axvline(cpos/1e6,color='grey',ls=':',lw=1,alpha=0.6); ax[1].set_ylabel(f'-log10 P ({disease})'); ax[1].set_xlabel(f'Chr{chrn} position (Mb)')
